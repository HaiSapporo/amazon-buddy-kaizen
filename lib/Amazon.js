...
class AmazonScraper {
    ...
    get userAgent() {
        {
            ...
            return `Mozilla/5.0 (${os[Math.floor(Math.random() * os.length)]}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${Math.floor(Math.random() * 4) + 100
    ...
    async startScraper() {
        ...
        await this.mainLoop().catch((err) => {
            // "No more products" không phải lỗi nghiêm trọng —
            // xảy ra khi Amazon trả về ít hơn 10 sản phẩm trên trang
            if (err && err.message === 'No more products') {
                console.log(`⚠️  ${err.message} — lưu ${this.collector.length} sản phẩm đã thu thập`);
            } else {
                throw err; // Lỗi thật sự thì ném tiếp ra ngoài
            }
        });

        console.log(`✅ Tổng số sản phẩm thu thập được: ${this.collector.length}`);
    ...
    /**
     * Sort and filter the result data
     */
    sortAndFilterResult() {
        if (this.scrapeType === 'reviews') {
            if (this.sort) {
        ...
        if (this.scrapeType === 'products') {
            ...
            // ── Loại bỏ ASIN trùng lặp, chỉ giữ lần xuất hiện đầu tiên ──
            const seenAsin = new Set();
            this.collector = this.collector.filter((item) => {
                if (seenAsin.has(item.asin)) return false;
                seenAsin.add(item.asin);
                return true;
            });

            // ── Chỉ giữ sản phẩm có giảm giá thực (discounted = true) ──
            this.collector = this.collector.filter((item) => item.price.discounted === true);

            // ── Chỉ giữ sản phẩm có savings_percent > 10% ──
            this.collector = this.collector.filter((item) => item.price.savings_percent > 10);
        }
            ...
            if (Object.keys(productJsonMeta.colorToAsin).length) {
                for (let item in productJsonMeta.colorToAsin) {
                    ...
                    const variant = {
                        ...
                        images: images,
                        title: item,
                        link: `https://www.amazon.com/dp/${productJsonMeta.colorToAsin[item].asin}/?tag=sansaleonhat-22&th=1&psc=1`,
            ...
            try {
                for (let item of variantsSelector[0].children) {
                    ...
                    variants.push({
                        ...
                        link: `https://www.amazon.com/dp/${variant.asin}/?tag=sansaleonhat-22&th=1&psc=1`,
    ...
    extractAlsoBought($) {
        ...
        for (let item of bought[0].children[0].children) {
            ...
            try {
                ...
                product.url = `https://www.amazon.com/dp/${product.asin}/?tag=sansaleonhat-22&th=1&psc=1`;
            } catch {
                // continue regardless of error
            }
    ...
    grabAsinDetails(body) {
        ...
        const output = {
            ...
            categories: [],
            asin: this.asin,
            url: `${this.mainHost}/dp/${this.asin}/?tag=sansaleonhat-22`,
            reviews: {
                total_reviews: 0,
                rating: 0,
    ...
    grabProduct(body, p) {
        ...
        for (let i = 0; i < productList.length; i++) {
            ...
            scrapingResult[asin] = {
                ...
                url: `${this.mainHost}/dp/${asin}/?tag=sansaleonhat-22`,
                score: 0,
                sponsored: false,
                amazonChoice: false,
                bestSeller: false,
        ...
        for (let key in scrapingResult) {
            console.log(`Product Name: ${scrapingResult[key].title}`);
            this.collector.push(scrapingResult[key]);
        }
    ...
    validateRating(item) {
        if (this.scrapeType === 'products') {
            // Nếu sản phẩm không có rating (= 0), cho qua — không filter
            if (item.reviews.rating === 0) return item;
